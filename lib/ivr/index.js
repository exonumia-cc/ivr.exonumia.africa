...
rootMediaDirectories.forEach(rootMediaDirectory => {
    ...
    console.log(`https://${ivrDomain}/africas-talking/${rootMediaDirectory}`)
})
    ...
    if (indexFile) {
        if (contentPathFiles.length > 1) {
            ...
            const gather = voiceResponse.gather({
                timeout: 20,
                action: `https://${ivrDomain}${request.baseUrl}${request.path}${subDirectory}`,
                numDigits: 1,
            })
            ...
            if (contentPathFiles.length == 1) {
                ...
                const gather = voiceResponse.gather({
                    timeout: 20,
                    action: `https://${ivrDomain}${request.baseUrl}${request.path}`,
                    numDigits: 1,
                })
...
module.exports.produceIVRVoiceResponse = function (request) {
